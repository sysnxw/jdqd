      ...
      switch (vo.action) {
        ...
        case "produce_position_info_v2":
          if (vo.data.material_name !== '') {
            console.log(`【${vo.data.position}】上正在生产【${vo.data.material_name}】，可收取 ${vo.data.procedure.produce_num} 份`)
            if (vo.data.procedure.produce_num !== '') {
        ...
        case "get_package":
          if (vo.code === '200' || vo.code === 200) {
            $.products = vo.data.material_name
          ...
          for (let benefit of vo.data) {
            if (benefit.type === 1) {
              ...
              if (parseInt(benefit.setting.beans_count) === bean &&
                ...
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                ...
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                client.send(`{"msg":{"type":"action","args":{"benefit_id":7},"action":"to_exchange"}}`)
                	client.send(`{"msg":{"type":"action","args":{"benefit_id":9},"action":"to_exchange"}}`)
                await $.wait(1000)
              }
            }
          }
  ...
  return new Promise(async resolve => {
    const options = {
      ...
      "headers": {
        ...
        "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : "jdapp;android;9.5.2;11;network/wifi;Dalvik/2.1.0 (Linux; U; Android 11; TAS-AN00 Build/TAS-AN00)") : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;android;9.5.2;11;network/wifi;Dalvik/2.1.0 (Linux; U; Android 11; TAS-AN00 Build/TAS-AN00)")
      }
    }
