...
let JD_DailyBonusPath = "./lxk0301_live_redrain.js";
let outPutUrl = './';
...
!(async() => {
  ...
  for (let i =0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      $.nickName = '';
      //await TotalBean();
      console.log(`*****************开始京东账号${$.index} ${$.nickName || $.UserName}整点京豆雨******************\n`);
...
async function execSign() {
  ...
  try {
    ...
    // }
    await exec(`${process.execPath} ${JD_DailyBonusPath}`);
    //const notifyContent = await fs.readFileSync(resultPath, "utf8");
    console.log(`??????????????????????LOG记录??????????????????????\n${notifyContent}\n??????????????????LOG记录??????????????????????`);
...
async function downFile () {
  ...
  await downloadUrl();
  if ($.body) {
    url = 'https://raw.githubusercontent.com/ljqailym/jd_scripts/master/lxk0301_live_redrain.js';
  } else {
    url = 'https://raw.githubusercontent.com/wulove/my_scripts/master/lxk0301_live_redrain.js';
  }
  try {
    const options = { }
    ...
    console.log(`lxk0301_live_redrain.js文件下载完毕\n\n`);
  } catch (e) {
    console.log("lxk0301_live_redrain.js 文件下载异常:" + e);
  }
}
...
//  }
  ...
  try {
    ...
    console.log("写入文件异常:" + e);
  }
}

function downloadUrl(url = 'https://raw.githubusercontent.com/ljqailym/jd_scripts/master/lxk0301_live_redrain.js') {
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          await $.http.get({url: `https://raw.githubusercontent.com/ljqailym/jd_scripts/master/lxk0301_live_redrain.js`, timeout: 10000}).then((resp) => {
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    JD_DailyBonusPath = process.env.TENCENTCLOUD_RUNENV === 'SCF' ? '/tmp/lxk0301_live_redrain.js' : JD_DailyBonusPath;
